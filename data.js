...
const itemData = [
    ...
    },
    {
        ...
        tech: '暗影操控器（四本）',
        features: [
            '落地烹饪站：激活打开「真饭桶」界面，从联动范围内的[[白熊冰箱]]、盐盒、原版冰箱等取食材并送入附近空锅烹饪',
            '联动半径：默认约 12（世界单位，模组可配置），与识别冰箱、校验取料、搜寻可用锅一致',
            '锅类型档位：普通烹饪锅 / 大厨便携锅 / [[百宝炊具]]（预测与入锅逻辑与对应配方表一致）',
            '多排预设与料理图鉴：类似[[饭桶]]的方案键；支持原版与模组料理书签，模组料理页可「刷新模组料理」',
            '独占使用：同一时间仅一名玩家可操作该真饭桶；离开世界会自动关闭动画状态',
            '锤击可拆除（有工作量）'
        ],
        hasDetail: true
    },
